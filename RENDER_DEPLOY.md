...
```
GitHub Repo (no API keys — config.js is gitignored)
        ↓ git push
...
|-----|-------|-------------------|
| `SUPABASE_URL` | `https://YOUR-PROJECT-ID.supabase.co` | Supabase → Settings → API → Project URL |
...
| `js/config.js` | Real Supabase credentials (generated by build.sh) | ❌ No (gitignored) |
